...
export const Hero: React.FC = () => {
  ...
  return (
    <section
      id="hero"
      className="relative min-h-[calc(100vh-73px)] flex flex-col justify-center pt-8 pb-16 md:pt-12 md:pb-20 overflow-hidden scroll-mt-20"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-10 items-center">
          ...
          <div className="lg:col-span-6 flex flex-col items-center lg:items-start text-center lg:text-left">
            ...
            <p className="text-xs font-medium text-slate-400 flex items-center gap-1.5 animate-blur-in-up delay-400">
              ...
              <span>
                Try the classification without an account &mdash; instant demo
              </span>
            </p>
          </div>

          {/* Right Column: Interactive Viewfinder Preview Frame with In-Frame Shutter */}
            ...
            >
              ...
              <div className="px-5 py-3.5 bg-slate-50/90 backdrop-blur-sm border-b border-slate-200/70 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span
                    className={`w-2.5 h-2.5 rounded-full ${scanState === "scanning" ? "bg-[#0066ff] animate-ping" : "bg-slate-300"}`}
                  ></span>
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full mr-0.5 ${scanState === "complete" ? "bg-emerald-600" : "bg-[#0066ff] animate-ping"}`}
                  ></span>
                  {scanState === "scanning"
                    ? "Scanning..."
                    : scanState === "complete"
              ...
              {/* Viewfinder Viewport */}
              <div className="relative min-h-[330px] sm:min-h-[360px] bg-slate-50/70 backdrop-blur-sm p-6 sm:p-8 flex flex-col items-center justify-center text-center select-none overflow-hidden">
                {/* Geometric Corner Guides with Active Laser Reticles */}
                <div
                  className={`absolute top-5 left-5 w-7 h-7 border-t-2 border-l-2 rounded-tl-lg pointer-events-none transition-all duration-300 ${scanState === "scanning" ? "border-[#0066ff] scale-110" : "border-[#0066ff]"}`}
                />
                <div
                  className={`absolute top-5 right-5 w-7 h-7 border-t-2 border-r-2 rounded-tr-lg pointer-events-none transition-all duration-300 ${scanState === "scanning" ? "border-[#0066ff] scale-110" : "border-[#0066ff]"}`}
                />
                <div
                  className={`absolute bottom-5 left-5 w-7 h-7 border-b-2 border-l-2 rounded-bl-lg pointer-events-none transition-all duration-300 ${scanState === "scanning" ? "border-[#0066ff] scale-110" : "border-[#0066ff]"}`}
                />
                <div
                  className={`absolute bottom-5 right-5 w-7 h-7 border-b-2 border-r-2 rounded-br-lg pointer-events-none transition-all duration-300 ${scanState === "scanning" ? "border-[#0066ff] scale-110" : "border-[#0066ff]"}`}
                />
                ...
                {scanState === "complete" && (
                  <div className="flex flex-col items-center justify-center animate-blur-in w-full max-w-sm py-2">
                    ...
                    <p className="font-sans text-xs text-slate-500 mb-4">
                      Pupil Clarity:{" "}
                      <strong className="text-slate-800">Clear (98%)</strong>{" "}
                      &bull; Gill Tint:{" "}
                ...
                {/* STATE 3: IDLE STATE WITH IN-FRAME SCAN TRIGGER BUTTON */}
                {scanState === "idle" && (
                  <div className="flex flex-col items-center justify-center w-full max-w-sm">
                    {/* Viewfinder Mode Switcher Pills */}
                    <div className="inline-flex p-1 rounded-full bg-white border border-slate-200 shadow-xs mb-5 backdrop-blur-xs">
                      <button
                    ...
                    {/* Main Target Visual Icon */}
                    <div className="w-16 h-16 rounded-2xl bg-white border border-slate-200 shadow-md flex items-center justify-center text-[#0066ff] mb-3 transition-all duration-300 group-hover:scale-105 group-hover:shadow-lg group-hover:shadow-blue-500/20">
                      {activePlaceholderTab === "camera" ? (
                        <Camera size={30} />
                      ) : (
                        <Upload size={30} />
                    ...
                    <h2 className="font-heading font-extrabold text-base text-slate-900 mb-1 tracking-tight">
                      {activePlaceholderTab === "camera"
                        ? "Position Fish in Frame"
                        : "Select Image for Analysis"}
                    </h2>
                    <p className="font-sans text-xs text-slate-500 max-w-xs mb-5 leading-relaxed">
                      Align fish eyes or gills inside the reticles for instant AI classification.
                    </p>

                    {/* Direct In-Viewfinder Tactile Scan Trigger */}
                    <button
                      onClick={handleStartLiveScan}
                      className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 px-6 py-3 rounded-2xl bg-[#0066ff] hover:bg-[#0052cc] text-white font-heading text-xs font-bold uppercase tracking-wider shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 hover:scale-[1.02] transition-all cursor-pointer group/btn"
                    >
                      <Play size={14} className="fill-white transition-transform group-hover/btn:scale-110" />
                      <span>
                        {activePlaceholderTab === "camera"
                          ? "Capture & Analyze"
                          : "Upload & Analyze"}
